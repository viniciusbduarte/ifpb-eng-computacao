...
void twoSum(int target, LISTA *lst);
...
void twoSum(int target, LISTA *lst) { 
    /*
     * Descrição:
     * Retorna os indices dos valores que somam 9 
     */
    struct sNODE *aux1 = lst->ini;
    
    int indice_aux1 = 0;
    int paresTarget = 0;
    
    while (aux1) {
        struct sNODE *aux2 = aux1->prox;  // Reinicializa aux2 para cada novo aux1
        int indice_aux2 = indice_aux1 + 1;
        
        while (aux2) {
            if ((aux1->dado + aux2->dado) == target) {
                printf("A soma dos indices %d e %d sao %d\n", indice_aux1, indice_aux2, target);
                paresTarget++;
            }
            aux2 = aux2->prox;
            indice_aux2++;
    ...
    if (!paresTarget) printf("Nenhum par de numeros nessa lista somam %d.\n", target);
